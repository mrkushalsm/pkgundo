...
**Known limitation — mutation capture on btrfs:** the auto-detect hooks above (install/remove reminders) work on any filesystem, but the underlying mutation-*capture* engine (which records what a tracked app actually writes under `$HOME`) currently doesn't work on **btrfs** — Fedora's default root filesystem. The kernel's `fanotify_mark(FAN_MARK_FILESYSTEM | FAN_REPORT_FID)` call this relies on fails with `EXDEV` on every btrfs path on affected kernels (confirmed via a standalone repro against a loopback ext4 filesystem on the same machine/kernel, which worked fine), so a package tracked on a btrfs-rooted system will show correctly in `pkgundo tracked` and get correct removal reminders, but `pkgundo inspect`/`untrack --rollback` will report zero mutations regardless of what the app actually did. Works as expected on ext4 (Arch, Debian, and most other distros' defaults).
